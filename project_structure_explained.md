...
**Pre-computed vector embeddings** of the MAITRI portal website content. Generated by the ingestion script (`src/scripts/ingest.ts`). Contains document chunks with their BGE-M3 embeddings for semantic search. Used as a supplementary knowledge source alongside the FAQ.
...
Format: `## Q: Question` followed by `A: Answer`. This file is **read at runtime** by the chatbot and serves as the ONLY source of truth for answers.
...
#### [chatService.ts](file:///c:/Users/Ajinkya/Desktop/MAITRI_BOT/src/services/chatService.ts) *(813 lines — the heart of the bot)*
...
3. **Dual LLM Strategy**:
   - **Primary: Google Gemini 2.0 Flash** — Single API call that answers from FAQ AND translates to user's language. Includes 429 rate-limit retry logic.
   - **Fallback: Ollama (qwen2.5:3b)** — Local LLM used when Gemini is unavailable. Uses LangChain's `ChatOllama`.

4. **Response Caching** — Persistent response cache (`data/response_cache.json`):
   - Up to 500 cached responses
   - Debounced disk writes (every 60s)
   - Cache key includes language + question + recent history
   - Survives server restarts

5. **Vector Search (RAG)** — Loads `vector_store.json` into memory, generates query embeddings via Ollama BGE-M3, computes cosine similarity, returns top-3 matches as supplementary context

6. **Greeting Detection** — Intercepts greetings in English/Hindi/Marathi and responds with a welcoming message in the user's language

7. **Form-Filling Flow** — Conversational form filling:
...
8. **HTML Formatting** (`formatReplyToHtml`) — Converts markdown-style responses to safe HTML (links, lists, escape XSS)

9. **Warm-up** — Pre-loads all caches on server start for fast first request

10. **Translation Fallback** — If Gemini single-shot fails, can translate via MyMemory free API
...
sequenceDiagram
    ...
    Note over User: Text Chat Flow
    User->>Node: POST /api/chat/message
    Node->>Node: Detect language
    Node->>Gemini: FAQ + prompt (answer in user's language)
    Gemini-->>Node: Native language answer
    Node-->>User: HTML reply

    Note over User: Voice Chat Flow
    User->>Node: POST /api/voice (audio)
    Node->>Python: Proxy to :8000
    Python->>Sarvam: STT (audio → text)
    Sarvam-->>Python: Transcript
    Python->>Node: POST /api/chat/message
    Node->>Gemini: Answer in native language
    Gemini-->>Node: Reply
    Node-->>Python: Reply text
    Python->>Sarvam: TTS (text → audio)
    Sarvam-->>Python: WAV audio
    Python-->>Node: JSON + base64 audio
    Node-->>User: Audio response
```
